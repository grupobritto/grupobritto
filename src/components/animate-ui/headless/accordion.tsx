...
function AccordionButton<TTag extends React.ElementType = 'button'>(
  ...
  return (
    <DisclosureButton
      ...
      {...rest}
      className={cn(
        'flex w-full flex-1 items-center justify-between py-4 text-start font-medium hover:underline',
        className,
      )}
    >
      {(bag) => (
...
function AccordionPanel<TTag extends React.ElementType = 'div'>(props: AccordionPanelProps<TTag>) {
  ...
  return (
    <DisclosurePanel data-slot="accordion-panel" {...rest} as={as as React.ElementType}>
      {(bag) => (
